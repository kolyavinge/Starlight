...
Vector3 VectorCalculator::GetMinVector(Collection<Vector3>& vectors)
{
...
Vector3 VectorCalculator::GetMaxVector(Collection<Vector3>& vectors)
{
    if (vectors.GetCount() == 0) throw ArgumentException();
...
void VectorCalculator::SwapXY(Collection<Vector3>& vectors)
{
...
void VectorCalculator::SwapXZ(Collection<Vector3>& vectors)
{
    for (int i = 0; i < vectors.GetCount(); i++)
    {
        Vector3& v = vectors[i];
...
void VectorCalculator::SwapYZ(Collection<Vector3>& vectors)
{
    for (int i = 0; i < vectors.GetCount(); i++)
    {
        Vector3& v = vectors[i];
